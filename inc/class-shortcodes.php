...
class Shortcodes implements Hookable {
    ...
    public function openagenda_filter_choice( $atts = array(), $content = null, $tag = 'openagenda_filter_choice' ){
        ...
        $defaults = array(
            'id'        => 'choice',
            // 'title'     => '',
            // 'filter'    => 'openagenda_filter_choice',
            // 'tag_group' => '',
            // 'tags'      => '',
            'field'     => '',
            'additional_field' => '',
        ...
        if( ! empty( $atts['field'] ) ){
            $params = array(
                'type' => 'choice',
                'name' => 'additional_field' === $atts['field'] && ! empty( $atts['additional_field'] ) ? sanitize_text_field( $atts['additional_field'] ) : sanitize_text_field( $atts['field'] ),
            );
